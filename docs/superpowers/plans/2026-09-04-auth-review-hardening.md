...
- Add an explicit runtime/CLI `--insecure-development` switch.
- Make Job, file transfer, tunnel, and shell requests reject missing Human
  Authorization by default.
- Keep only the explicit development switch as the compatibility escape hatch;
  no automatic fallback based on missing human files.
- Add unit tests for missing authorization and for the explicit development
  mode.

## 2. Separate Sister and Human revocation domains

- Add a typed revocation subject (`Sister` or `Human`) to the signed
  `RevocationRecord` and its canonical signing bytes.
- Update persistence deduplication and lookup to include subject kind.
- Update CLI revoke output/input and all callers/tests.
- Add regression tests proving equal serials in the two domains do not collide.

## 3. Enforce Human revocation and live Sister revocation

- Check the typed Human membership revocation before accepting Job and stream
  authorizations.
- Replace startup-only Sister serial snapshots with a local revocation-store
  lookup at every authenticated session decision.
- Add focused tests for revoked Human membership and a post-start Sister
  revocation being rejected.

## 4. Make Transfer v2 authorizations lifetime-safe

- Expose a repository-native way to issue a fresh authorization for each
  Prepare/Chunk/Finalize request, using a new nonce and bounded five-minute
  window.
- Ensure parallel workers never reuse the Prepare nonce for Chunk requests.
- Add tests that distinguish nonce freshness from authorization identity and
  constraints.

## 5. Align Role permissions with the approved model

- Owner: all permissions.
- Admin: `network.invite`, `network.revoke`, `sister.inspect`.
- Operator: `sister.inspect`, `job.submit`, `job.cancel`, `file.send`,
  `tunnel.open`, `shell.open`.
- Add exhaustive mapping tests.

## 6. Add adversarial Testament coverage and documentation

- Add a dedicated `security-verify` command and scenarios for missing human
  authorization, wrong/expired/revoked authorization, serial-domain
  isolation, and live revocation.
- Update CI and the authorization/membership docs to describe fail-closed
